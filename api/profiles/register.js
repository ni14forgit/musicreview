...
registerRouter.post('/', async (req, res) => {
    ...
    let { name, email, password, profilephoto, songs, genres, professions, spotify, soundcloud, instagram } = req.body;
    console.log({
        name,
        email,
        password,
    ...
    const registerUser = async () => {
        ...
        RETURNING id, password`,
                [name, email, hashedPassword, profilephoto.path_to_profile_photo, spotify, instagram, soundcloud]
            );
            return results;
        } catch (err) {
            throw err;
    ...
    const insertSongs = async (id) => {
        ...
        for (iterateOverSongs = 0; iterateOverSongs < songs.songs.length; iterateOverSongs++) {
            console.log('inserting song');
            pool.query(
                `INSERT INTO songs (user_id, url, title)
                VALUES ($1, $2, $3)`,
                [id, songs.songs[iterateOverSongs].path_to_song, songs.songs[iterateOverSongs].songname],
                (err, results) => {
                    if (err) {
                        console.log(err);
                        throw err;
    ...
    const insertProfessions = async (id) => {
        pool.query(
          ...
          VALUES ($1, $2, $3, $4, $5)`,
            [id, professions.singer, professions.songwriter, professions.audio_engineer, professions.producer],
            (err, results) => {
                if (err) {
                    console.log(err);
                    throw err;
